...
\end{verbatim}              %$
This creates a directory named {\tt BUCKy-1.2} with subdirectories
{\tt BUCKy-1.2/data} and {\tt BUCKy-1.2/src}.
...
(see below). {\sc not} created by default.\\
...
tree topologies.\\
{\tt .pairs}&Gives a $k$ by $k$ similarity matrix, $k$ being the number 
...
a list of $k$ trees. Trees are given by their indices. The correspondance
...
directory\\ \verb+$BUCKY_HOME/BUCKy-1.2/data/example1/+ %$
...
\begin{verbatim}
bucky $BUCKY_HOME/BUCKy-1.2/data/example1/ex?/ex.in
\end{verbatim}%$
...
bucky -n 1000000 -a 5 -s1 745203 -s2 905423 --do-not-use-update-groups 
                                $BUCKY_HOME/BUCKy-1.2/data/example1/ex?/ex.in
bucky -n 1000000 -a 5 --do-not-use-update-groups -s1 4948537 -s2 8764223
                                $BUCKY_HOME/BUCKy-1.2/data/example1/ex?/ex.in
\end{verbatim}

After the first run, a look at the file {\tt run1.gene} shows that all 
genes give a 100\% estimated posterior probability to tree 0 and 0\% estimated
posterior probability to other trees. However, after the second run we
see that tree 1 receives 100\% estimated posterior probability by all loci, 
instead of tree 0. So the two runs are in very strong disagreement. 
...
\verb+$BUCKY_HOME/BUCKy-1.2/data/yeast/+ %$ 
...
$n=1,000,000$ updates, $c=4$ chains (one cold and 3 hot chains),
saving samples once every 1000 updates, and for keeping a similarity 
matrix among genes, one would type (on a single line)
\begin{verbatim}
bucky -a 2.5 -n 1000000 -c 4 --create-sample-file --calculate-pairs
                           $BUCKY_HOME/BUCKy-1.2/data/yeast/y???/run2.nex.in
\end{verbatim}%$
